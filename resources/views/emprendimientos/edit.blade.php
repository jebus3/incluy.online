@extends('layouts.app')
@section('title', 'Editar Emprendimiento — Incluy Admin')
@section('content')
<div class="mb-6">
    <a href="/emprendimientos" class="text-sm text-[#6B7C93] hover:text-[#004494]">← Volver</a>
    <h1 class="text-2xl font-bold text-[#0A0E27] mt-2">Editar: {{ $emprendimiento->nombre_marca }}</h1>
</div>
<div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 max-w-2xl">
    <form method="POST" action="/emprendimientos/{{ $emprendimiento->id }}" class="space-y-4">
        @csrf @method('PUT')
        @include('emprendimientos._form')
        <div class="pt-2">
