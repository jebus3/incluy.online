...
<body class="flex h-screen bg-[#F0F4F8] text-[#0A0E27]" x-data>
    ...
    <aside class="w-64 flex-shrink-0 flex flex-col" style="background: linear-gradient(180deg, #004494, #3C2D6D)">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1">
            <a href="/"
               class="flex items-center gap-3 px-3 py-2 rounded-lg text-white/80 hover:text-white hover:bg-white/10 transition {{ request()->is('/') ? 'bg-white/15 text-white' : '' }}">
            ...
            <div class="pt-2">
                ...
                <a href="/directorio"
                   class="flex items-center gap-3 px-3 py-2 rounded-lg text-white/80 hover:text-white hover:bg-white/10 transition {{ request()->is('directorio*') && !request()->is('directorio/profesionales*') ? 'bg-white/15 text-white' : '' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                    Organizaciones
                </a>
                <a href="/directorio/profesionales"
                   class="flex items-center gap-3 px-3 py-2 rounded-lg text-white/80 hover:text-white hover:bg-white/10 transition {{ request()->is('directorio/profesionales*') ? 'bg-white/15 text-white' : '' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                    Profesionales
                </a>
                <a href="/emprendimientos"
                   class="flex items-center gap-3 px-3 py-2 rounded-lg text-white/80 hover:text-white hover:bg-white/10 transition {{ request()->is('emprendimientos*') ? 'bg-white/15 text-white' : '' }}">
            ...
            <div class="pt-2">
                ...
                <a href="/admin-usuarios"
                   class="flex items-center gap-3 px-3 py-2 rounded-lg text-white/80 hover:text-white hover:bg-white/10 transition {{ request()->is('admin-usuarios*') ? 'bg-white/15 text-white' : '' }}">
